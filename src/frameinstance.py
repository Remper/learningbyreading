...
def get_frame_instances(variables, semantics, thematic_roles):
    ...
    if config.get('semantics', 'module') == 'boxer':
        ...
        for variable, frame in semantics['frames'].iteritems():
            ...
            counter += 1
            if counter % 1000 == 0:
                log.info("Processed %.0fk frames (%.2fs)" % (float(counter)/1000, time.time() - timestamp))
...
def get_frame_triples(frame_instances):
    ...
    for frame_instance_id, frame_instance in frame_instances.iteritems():
        ...
        counter += 1
        if counter % 10000 == 0:
            log.info("Processed %.0fk out of %d (%.2fs)" % (float(counter)/1000, len(frame_instances), time.time() - timestamp))
